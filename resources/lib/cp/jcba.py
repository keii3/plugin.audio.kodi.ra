...
class Params:
    ...
    # ファイルパス
    DATA_PATH = os.path.join(Const.DATA_PATH, 'jcba') # cache/data/jcba/
...
class Jcba(Params):
    ...
    def getSchedule(self, calendar='', fp=''):
        ...
        params = {
            'orderBy':'startTime',
            'timeMin':datetime.strftime(datetime.now(pytz.utc),'%Y-%m-%dT%H:%M:%SZ'),
            'timeMax':datetime.strftime(datetime.now(pytz.utc)+timedelta(days=1),'%Y-%m-%dT%H:%M:%SZ'),
            'timeZone':'Asia/Tokyo',
            'maxResults':'24', # >='3'
            'singleEvents':'true',
    ...
    def getProgramData(self, renew=False):
        ...
        #dom = convert(parse(data))
        #st = dom['settings']['setting']
        # ids = [{'id': s.get('@id')} for s in st if s.get('@id')[0:5] == 'jcba_' and s.get('#text') == 'true'] # 聴取対象局リスト
        ids = self.getActiveStation(self.STN)
        if ids:
