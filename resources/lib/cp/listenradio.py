...
class Listenradio(Params, Jcba):
    ...
    def getProgramData(self, renew=False):
        ...
        # ids = [{'id': s.get('@id')} for s in st if s.get('@id')[0:5] == 'lisr_' and s.get('#text') == 'true'] # make StationId list
        ...
        if ids:
            for id in ids:
                ...
                data = read_file(fp)
                if data is not None:
                    data = convert(json.loads(data), True)
                    ...
                    for p in pp:
                        ft = str(p.get('StartDate')) + "00"
                        ftl = str(ft[8:12])
                        to = str(p.get('EndDate')) + "00"
                        tol = str(to[8:12])
                        progs.append({
                            'ft': ft,
                            'ftl': ftl,
